...
This library provides a dynamically fields selection while prefetching them on database to improve to request performance in Django REST Framework.
...
It's similiar to [drf-dynamic-fields](https://github.com/dbrgn/drf-dynamic-fields) and [drf-flex-fields](https://github.com/rsinger86/drf-flex-fields) great libraries, with the main aim of improving the poor database perfomances reached when the fields to return are evaluted runtime, and thus it's not possibile to hardcode a prefetchable queryset.
It uses the capabaility provided by the "Prefetch" class of Django ORM to build the queryset on the fly.
> I'd liked to name this library *dfr-extra-fields* but the name had been already stolen :disappointed:
...
Imagine that you have a customer API that returns all its orders and inside each order, the related article and the customer who placed the order itself.
...
``GET /api/customers/1``
    ...
    "orders": [
        {   
            ...
            "customer": 1,  # customer name needed
            "article": {
                "id": 1,  # unnecessary
                "code": "PIZZA"
...
This response could have this two problems: unwated fields (we don't want ids, any customer's data except for the name, and so on) and speed, because we have camed across the N+1 problems.

### Standard solution
We can fix those issues by modifying the queryset in the view, adding all the prefetch stuff and creating another custom serializer to remove to annoing uncessary fields. We must do this every time we want return an even sligthly different response. In this case we want the article inside order, but in other case we just want the plain order's data without any other nested field.

### Drf-Eager-Fields

Using the *drf-eager-fields* you can reach this result without modifying the view or the serializer.
...
First of all, you have to set your serializer with the eager mixing class and, if you want to use the extra fields, you must set the `extra` class property.
> `extra` dictionary is defined as a function with the **@classproperty** decorator, to permit the import of nested serializer while avoiding the circular import problem. Until you face this problem, you can handle the `extra` prop as a normal class attribute.
...
If you want to add an extra fields, which is defined in the ``extra`` property, here we go
...
With ``dotted`` notation, you can add the nested extra fields, if they are previously defined in the serializers extra dict:
...
and leave only the model's flat properties the serializer.Meta 'fields' attribute. 
...
- inside the request, as query params. This overrides the default parameters in the view:

        GET /api/articles/?extra=customer,fields=code,customer.name,exclude=...

- inside the GET request, as JSON body. This JSON represent the wanted response structure and overrides other paramas set in view and the query params string:
...
The problem with the dynamic response, is how to eager loading (prefethcing)
all the necessary data without leading ourself to the famous N+1 nigthmare.
This library take the capability of the Django ``Prefetch`` class to deal with it.
...
If you set the ``prefetch`` attribute, even for the nested fields, this library will compose the prefetched queryset for you.
### One step further..
the prefetch attribute can accept a bool or Prefetch object where you
can set you custom queryset. Let me show you an example with the article API: image that you just want to show the related orders, but only the last 10 orders, and of course, prefetch them all.
...
you will get only the last 10 prefetch orders, order by the date. :bomb:
> Sorry for the complex queryset but in Django you cannot limit the queryset directly, so you have to nest a subquery the get the latest ids.
